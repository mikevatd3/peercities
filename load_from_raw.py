...
).to_csv(prepped_dir / "work_hours.csv", index=False)
...
).to_csv(prepped_dir / "population.csv", index=False)
...
).to_csv(prepped_dir / "temperature.csv", index=False)

compiled_filename = "countes_with_details_1980.csv"


df = pd.concat(
    [
        pd.read_csv(prepped_dir / item).set_index(["state_code", "county_code"])
